...
__all__ = [
    # skimage method: 4-neighbors, interpolate diagionals
    "nms2d_sk", "nms3d_sk",
    # Gonzalez-Woods method: 8-neighbors, no interpolation
    "nms2d_gw", "nms3d_gw",
]
...
def nms2d_sk(S, O, S_threshold=1e-6):
...
def nms3d_sk(S, O, S_threshold=1e-6):
...
def nms2d_gw(S, O, S_threshold=1e-6, suppress=True):
    """ non-maximum suppresion of S[ny, nx]
    :param S: saliency
    :param O: orientation (tangent)
    :param S_threshold: threshold on S, usually a small value
...
@numba.njit(parallel=True)
def nms3d_gw(S, O, S_threshold=1e-6, suppress=True):
